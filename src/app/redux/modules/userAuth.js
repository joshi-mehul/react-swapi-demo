...
const RECEIVED_TEMP_USER        = 'RECEIVED_TEMP_USER';
...
export default function (state = initialState, action) {
  ...
  case RECEIVED_USER_REGISTER:
    ...
    const {user} = action;
    if(tempUser.name === user.name && tempUser.birthYear === user.birth_year) {
      auth.setUserInfo(user);
      return {
        ...state,
        name:        action.user.name,
        birthYear:       action.user.birthYear,
        tempUser: null,
        error:           null
      };
    }
...
// login sucess:
// //////////////////
export function setTempUser(user = emptyUser, time = moment().format(dateFormat)) {

  return {
    type: RECEIVED_TEMP_USER,
    time,
    user
  };
}
