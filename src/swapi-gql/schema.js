...
const queryType = new GraphQLObjectType({
  ...
  fields: () => {
    ...
    const search = mapValues(mapKeys(graphQLObjectTypes, (value, key) => {
        // Name the query people_search vehicles_search etc. not sure what the standard should be
        // here. We could also adopt the Relay node spec
        return key + "_search";
    }), (type, typeQueryName) => {
        const restName = typeQueryName.split('_')[0];
        ...
        return {
            type: new GraphQLList(type),
            description: `Search ${restName}.`,
            args: {
                value: { type: GraphQLString }
            },
            resolve: (_, { value }) => {
                return searchDoc(restName, value);
            },
        }
    });
...
function searchDoc(restName, search) {
    return restLoader.load(`https://swapi.co/api/${restName}/?search=${search}`).then((data)=>{
      return data.results;
    })
}
